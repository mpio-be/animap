...
make_moviemap <- function(dir, outnam, outdir, nframes = nrow(frmss) , movie_length_secs = 25) {
  ...
  call =
    glue::glue('cd {dir} && ffmpeg -r {frate} -pattern_type glob -i "*.png" -c:v libx264 -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -pix_fmt yuv420p -preset veryslow -crf 1 {outnam}')
...
makefrm <- function (d, bytime = "15 mins", byid = "id") {
  d[, .(date = seq(as.POSIXct(as.Date(min(date))), as.POSIXct(as.Date(max(date))), 
      by = bytime)), by = byid]
}